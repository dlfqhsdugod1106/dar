...
from train_reverb.loss.paramloss import ParameterMatchLossFDN as ParameterMatchLoss
...
parser.add_argument('--lr', type = float, default = 1e-6, help = 'initial lr of optimizer')
...
for ep in range(init_ep, args.epochs):
    ...
    for i, dspeech in enumerate(tqdm(train_loader)):
        with torch.no_grad():
            gt_params_fdn = fdn_dec.generate_params()
            gt_params_sap_fb = sap_dec.generate_params()
            gt_feedback_sap = util.prod(sap(gt_params_sap_fb).transpose(-1, -2))

            params_svf_pre, pre = None, None

            gt_params_svf_post = svf_post_dec.generate_fdn_params()
            gt_post = util.prod(svf(gt_params_svf_post).transpose(-1, -2))
            gt_post = gt_post.transpose(-1, -2).unsqueeze(-2)

            gt_params_svf_fb = svf_fb_dec.generate_params()
            gt_fb_filter = util.prod(svfbell(gt_params_svf_fb).transpose(-1, -2))
            gt_fb_filter = gt_fb_filter * gt_feedback_sap
            gt_fb_filter = gt_fb_filter.transpose(-1, -2)

            gt_params_color = dict(pre = pre, post = gt_post, fb_filter = gt_fb_filter)

            ir = fdn({**gt_params_fdn, **gt_params_color})
            ir = ir[:, :args.ir_len]
            ir_norm = torch.sqrt(torch.sum(ir ** 2, -1, keepdim = True))
            ir = ir / ir_norm

            gt_params_fdn['fir'] = gt_params_fdn['fir'] / ir_norm
            gt_params_fdn['post_gain'] = gt_params_fdn['post_gain'] / ir_norm
        ...
        param_loss.init()
        loss = param_loss({**gt_params_fdn}, {**params_fdn}, '_fdn') + param_loss({**gt_params_sap_fb}, {**params_sap_fb}, '_sap')\
             + param_loss({**gt_params_svf_post}, {**params_svf_post}, '_post') + param_loss({**gt_params_svf_fb}, {**params_svf_fb}, '_fb')
        param_losses.append(loss.detach().item())
        loss.backward()
...
with torch.no_grad():
    ...
    for i, (ir, tspeech, dspeech, dspeech_idx, ) in enumerate(tqdm(test_loader)):
        ...
        for j in range(args.batch_size):
            ...
            sf.write(test_dir + task_str + str(j) + '_true_convolved_speech.wav', tcspeech, args.sr)
