...
class PyRoom3_saved(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.rdeq:
            ir = rdeq(ir, smoothing_range = self.rdeq_range)
        ir = ir / np.sqrt(np.sum(ir ** 2) + 1e-12)


        """ SPEECH """
        tspeech, dryspeech_cut, _ = self.get_speeches(ir)
        tspeech_2, _, _ = self.get_speeches(ir)
        _, _, dryspeech_cut_2 = self.get_speeches(np.zeros(int(self.sr * self.ir_len)))
        ...
        return ir, tspeech, dryspeech_cut, tspeech_2, dryspeech_cut_2

    def __len__(self):
        return len(self.ir_directory)

    def get_speeches(self, ir):
        ...
        dryspeech_cut_cut = dryspeech_cut[len(ir):]
        tspeech = tspeech[len(ir):]

        return tspeech, dryspeech_cut_cut, dryspeech_cut
