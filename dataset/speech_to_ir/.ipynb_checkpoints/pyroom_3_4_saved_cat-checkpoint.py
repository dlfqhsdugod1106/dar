...
# numpy!
def Generate_Dataset(ir_len = 2, noiseir_ratio = .3, sr = 16000, speech_len = 2.5, ism_max_order = 50, rdeq = True, rdeq_range = (1, 3), vali_dspeech_idx = False, time_alias = False):
    ...
    print("OPEN", len(test_open_2))

    vali_list = vali_alti + vali_ace + vali_open
    test_list = test_alti_1 + test_alti_2 + test_ace_1 + test_ace_2 + test_open_1 + test_open_2

    #random.shuffle(vali_list)
    #random.shuffle(test_list)
    #print(len(vali_list), len(test_list))

    #pickle.dump([vali_list, test_list], open('rir_dir.pickle', 'wb'))
    vali_list, test_list = pickle.load(open('rir_dir.pickle', 'rb'))
    print(len(vali_list), len(test_list))
    print(test_list[0])
    #print(len(hi[0]), len(hi[1]))

    train_ir = PyRoom3_saved(train_speech, 
                             sr = sr, 
                             ir_len = ir_len, 
                             noiseir_ratio = noiseir_ratio, 
                             speech_len = speech_len, 
                             rdeq = rdeq, 
                             rdeq_range = rdeq_range,
                             time_alias = time_alias)

    vali_ir = IRDataset(vali_list,
                        vali_speech,
                        out_sr = sr,
                        out_len = ir_len,
                        speech_len = speech_len,
                        get_speech_idx = vali_dspeech_idx,
                        aug = False,
                        time_alias = time_alias)

    #for i in range(len(test_list)):
    #    print(i // 4, i % 4, test_list[i])

    test_ir = IRDataset(test_list,
                        test_speech,
                        out_sr = sr,
                        out_len = ir_len,
                        speech_len = speech_len,
...
def Generate_Dataset_Param_Baseline(ir_len = 2, noiseir_ratio = .3, sr = 16000, speech_len = 2.5, ism_max_order = 50, rdeq = True, rdeq_range = (1, 3), vali_dspeech_idx = False, time_alias = False):
    train_speech, _, _ = get_speech_lists()
    trainspeech = DARParamSet(train_speech, sr = sr, ir_len = ir_len, speech_len = speech_len)
    return trainspeech
...
class PyRoom3_saved(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(ir) > self.sr * self.ir_len:
            if self.time_alias:
                ir = self.gt_time_alias(ir, int(self.sr * self.ir_len))
            else:
...
def BUTREVDB_SPLIT(directory = '/SSD/BUTREVDB/RIR_only', split_ratio = (.5, .2)):
