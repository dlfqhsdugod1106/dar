...
# numpy!
def Generate_Dataset(ir_len = 2, noiseir_ratio = .3, sr = 16000, speech_len = 2.5, ism_max_order = 50, rdeq = True, rdeq_range = (1, 3), vali_dspeech_idx = False, time_alias = False, seed = 1234):
    random.seed(seed)
    train_speech, vali_speech, test_speech = get_speech_lists()
    random.shuffle(test_speech)
    print(test_speech[:10])
...
class IRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        print(self.ir_directory_list[idx])
        print(self.speech_directory_list[idx])
        ...
        """ SPEECH """
        speech, speech_sr = sf.read(self.speech_directory_list[idx])
        ...
        if self.get_speech_idx:
            if self.time_alias:
                return crop_data, tspeech, dryspeech_cut, idx, data[:int(self.out_sr * self.out_len)]
            else:
                return crop_data, tspeech, dryspeech_cut, idx
        else:
            if self.time_alias:
