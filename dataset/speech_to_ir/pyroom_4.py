...
def Generate_Dataset(ir_len = 2):
    ...
    train_pyroom = PyRoom3(train_speech, ir_len = ir_len)

    vali_alti, test_alti_1, test_alti_2 = ALTI_SPLIT([vali_speech, test_speech, test_speech], split_ratio = (.5, .2), train_aug = False, out_len = ir_len, aug_const = 1)
    vali_ace, test_ace_1, test_ace_2 = ACE_SPLIT([vali_speech, test_speech, test_speech], split_ratio = (.5, .2), train_aug = False, out_len = ir_len, aug_const = 1)
    vali_but, test_but_1, test_but_2 = BUTREVDB_SPLIT([vali_speech, test_speech, test_speech], split_ratio = (.5, .2), train_aug = False, out_len = ir_len)
    vali_open, test_open_1, test_open_2 = OPENAIR_SPLIT([vali_speech, test_speech, test_speech], split_ratio = (.5, .2), train_aug = False, out_len = ir_len, aug_const = 1)
    vali_gh = GREATHALL(vali_speech, out_len = ir_len)
    vali_mardy = MARDY(vali_speech, out_len = ir_len)
    ...
    print(len(test_alti_1), len(test_alti_2))
    test_list = [test_ace_1, test_ace_2, test_but_1, test_but_2, test_open_1, test_open_2]
...
class PyRoom3(Dataset):
    ...
    def __getitem__(self, idx):
        if np.random.uniform() > self.noiseir_ratio:
            ...
            ir = rdnoise_ir(ir_len = self.sr * self.ir_len)
...
class IRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dspeech_total = rms_normalize(speech)
        tspeech_total = signal.convolve(rms_normalize(speech), data)

        speech_idx = random.randint(0, self.speech_dataset_len - 1)
        dryspeech_another, speech_sr = sf.read(self.speech_directory_list[speech_idx])
        
        if len(speech.shape) == 2:
            speech = speech[..., 0]

        data = data[:self.out_sr * self.out_len]

        dryspeech_cut = dryspeech_cut[len(data):]
        tspeech = tspeech[len(data):]

        return data, tspeech, dryspeech_cut, dspeech_total, tspeech_total, dryspeech_another

    def __len__(self):
        return int(len(self.ir_directory_list) * self.aug_const * self.sample_ratio)


def ALTI_SPLIT(speech_lists, directory = '/SSD/RIR/altiverb-ir', split_ratio = (.6, .2), aug_const = 3, train_aug = True, out_len = 2):
...
def ACE_SPLIT(speech_lists, directory = '/SSD/RIR/ACE', split_ratio = (.6, .2), train_aug = True, aug_const = 50, out_len = 2):
    total_directory_list = glob.glob(directory + '/*.wav')
...
def BUTREVDB_SPLIT(speech_lists, directory = '/SSD/BUTREVDB/RIR_only', split_ratio = (.6, .2), train_aug = True, out_len = 2):
...
def OPENAIR_SPLIT(speech_lists, directory = '/SSD/RIR/OpenAIR', split_ratio = (.6, .2), train_aug = True, aug_const = 10, out_len = 2):
