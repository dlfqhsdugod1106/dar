...
def fbir(ir, band = 'oct', sr = 16000, num_band = 10, order = 4):
    if band == 'bark':
        low, hi = bark_band2(sr)
    elif band == 'oct':
        low, hi = oct_band(sr)
    elif band == 'oct3':
        low, hi = third_oct_band(sr)
    elif band == 'linear':
        low, hi = linear_band(sr, num_band)
...
def third_oct_band(sr):
    """
    Returns Octave Band Cutoff Frequencies
    """

    freq = np.array([15.625, 19.686, 24.803, 31.250, 39.373, 49.606, 62.500, 78.745, 99.213,
    ...
    low, hi = freq / (2 ** (1/6)), freq * (2 ** (1/6))

    return low, hi


def linear_band(sr, num_band = 10):
    freq = np.linspace(0, sr / 2, num_band) 
    low, hi = freq[:-1], freq[1:]
    return low, hi
...
def rdeq(ir,
        fb_band = 'oct3',
        geq_gain_range = (-12, 12),
        num_band = 10,
        order = 4,
        smoothing_range = (1, 3)):

    fbed = fbir(ir, band = fb_band, num_band = num_band, order = order)
    num_fb = fbed.shape[0]
    
    geq_smoothing = np.random.randint(smoothing_range[0], smoothing_range[1])
    geq_gain_db = np.random.rand(num_fb + geq_smoothing - 1) * (geq_gain_range[1] - geq_gain_range[0]) + geq_gain_range[0] # uniform dist.
    geq_gain_smoother = np.hanning(geq_smoothing + 2)
    geq_gain_smoother = geq_gain_smoother / np.sum(geq_gain_smoother)
    geq_gain_db = np.convolve(geq_gain_db, geq_gain_smoother)[geq_smoothing:-geq_smoothing]
...
def rdeq2(ir,
    ...
    geq_gain_smoother = np.hanning(geq_smoothing + 2)
    geq_gain_smoother = geq_gain_smoother / np.sum(geq_gain_smoother)
    geq_gain_db = np.convolve(geq_gain_db, geq_gain_smoother)[geq_smoothing:-geq_smoothing] * np.sqrt(geq_smoothing)
    geq_gain = 10 ** (geq_gain_db / 20)
